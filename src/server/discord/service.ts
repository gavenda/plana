...
import { buildJoinAnnouncement, buildResolvedAnnouncement } from './components'
...
export async function announceJoin(member: GuildMember): Promise<Result<{ messageId: string }>> {
  ...
  if (!channel?.isTextBased()) {
    return err('unknown_channel', 'The configured announcement channel no longer exists.')
  }

  const roles = await getAssignableRoles(settings)
  const container = buildJoinAnnouncement(member, roles)
  ...
  try {
    ...
    return err('discord_error', 'Could not post the announcement message.')
  }
}
...
async function markAnnouncementResolved(options: {
  ...
  try {
    ...
    await message.edit({
      components: [buildResolvedAnnouncement({ member, roleName, actorName: actor.name, source })],
      flags: MessageFlags.IsComponentsV2,
    })
  } catch (error) {
...
export async function assignRole(input: {
  ...
  if (!allowed.includes(roleId)) {
    return err('role_not_assignable', 'That role is not one of the configured member roles.')
  }

  const guild = await getGuild()
  const member = await guild.members.fetch(memberId).catch(() => null)
  if (!member) return err('unknown_member', 'That member is no longer in the server.')

  const role = guild.roles.cache.get(roleId) ?? (await guild.roles.fetch(roleId).catch(() => null))
  if (!role) return err('unknown_role', 'That role no longer exists.')

  const me = guild.members.me ?? (await guild.members.fetchMe())
  if (!me.permissions.has(PermissionFlagsBits.ManageRoles)) {
    return err('missing_permissions', 'The bot is missing the Manage Roles permission.')
  }
  if (role.position >= me.roles.highest.position) {
    return err(
      'missing_permissions',
      `The bot's highest role must sit above ${role.name} in the role list.`,
    )
  }
  ...
  try {
    ...
    return err('discord_error', 'Discord rejected the role change.')
  }
...
export async function listPendingMembers(): Promise<Result<PendingMember[]>> {
  const settings = getSettings()
  if (!settings.triageRoleId) return err('not_configured', 'No triage role is configured.')
  ...
  if (!role) return err('unknown_role', 'The configured triage role no longer exists.')
...
export async function broadcast(input: {
  ...
  if (!channel?.isTextBased()) {
    return err('unknown_channel', 'That channel does not exist or is not a text channel.')
  }
  ...
  if (!canSend) {
    return err('channel_not_sendable', `The bot cannot post in #${channel.name}.`)
  }
  ...
  try {
    ...
    return err('discord_error', 'Discord rejected the message.')
  }
}
